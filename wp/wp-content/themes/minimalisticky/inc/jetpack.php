 ...
 */
function minimalisticky_jetpack_setup() {
	// Add theme support for Infinite Scroll.
	add_theme_support( 'infinite-scroll', array(
		'footer_widgets'=> 'sidebar-1',
                'container'     => 'post-archives',
		'render'	=> 'minimalisticky_infinite_scroll_render',
		'footer'	=> 'colophon',
                'post_per_page' => 8,
	) );
	...
	add_image_size( 'minimalisticky-logo', 200, 200 );
	add_theme_support( 'site-logo', array( 'size' => 'minimalisticky-logo' ) );
 ...
 */
function minimalisticky_jetpack_default_hide_cf( $hidden, $screen ) {
...
add_action( 'default_hidden_meta_boxes', 'minimalisticky_jetpack_default_hide_cf', 10, 2 );

/**
 * Custom render function for Infinite Scroll.
 */
function minimalisticky_infinite_scroll_render() {
	while ( have_posts() ) {
		the_post();
		if ( is_search() ) :
 ...
 */
function minimalisticky_the_site_logo() {
...
if( !function_exists( 'minimalisticky_social_menu' ) ) :
 ...
 */
global $minimalisticky_current_class;
$minimalisticky_current_class = 'odd';
function minimalisticky_odd_even_post_class( $classes ) {
    if( get_post_type( get_the_ID() ) === 'jetpack-testimonial' ) {
        global $minimalisticky_current_class;
        $classes[] = $minimalisticky_current_class;
        $minimalisticky_current_class = ( $minimalisticky_current_class == 'odd' ) ? 'even' : 'odd';
    }
    return $classes;
}
add_filter( 'post_class', 'minimalisticky_odd_even_post_class' );

/**
 * Move JetPack Share and Like buttons
 * @link https://jetpack.com/2013/06/10/moving-sharing-icons/
 */
function minimalisticky_jptweak_remove_share() {
...
add_action( 'loop_start', 'minimalisticky_jptweak_remove_share' );
    ...
    <?php if ( '' != get_the_post_thumbnail() ) : ?>
        <div class="index-post-thumbnail">
                <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'minimalisticky-featured-image' ); ?>
                </a>
        </div>
    <?php endif; ?>
...
<?php else : // Set the Featured Image as the Background Image on Archive Pages ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class(); ?> <?php
            if ( '' != get_the_post_thumbnail() ) { ?> style="background: white url(<?php echo esc_url( the_post_thumbnail_url( 'minimalisticky-featured-image' ) ); ?>);" <?php } ?>>
	...
	<div class="entry-content">
		<?php
                    ...
                    if ( is_archive() || is_home() || is_front_page() || is_page_template( 'page-templates/frontpage-portfolio.php' ) ) { // Makes EVERY Post on an index page an excerpt
                        minimalisticky_fancy_excerpt();
                    } else {
			the_content( sprintf(
				/* translators: %s: Name of current post. */
				wp_kses( __( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'minimalisticky' ), array( 'span' => array( 'class' => array() ) ) ),
				the_title( '<span class="screen-reader-text">"', '"</span>', false )
			) );
                    }
                    minimalisticky_jetpack_sharing();
        ...
        ?>          
            <header class="entry-header">
                    ...
                    <div class="entry-meta">
                            <?php minimalisticky_posted_on(); ?>
                    </div><!-- .entry-meta -->
                    <?php endif; ?>
            </header><!-- .entry-header -->

            <footer class="entry-footer group">
                    <?php minimalisticky_entry_footer(); ?>
            </footer><!-- .entry-footer -->
        <?php 
            endif;
        ?>
